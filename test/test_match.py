...
def test_equal_match_is_superset():
    match = Match(
        src=LPM(IPv4Address("192.168.1.42"), prefix=32), 
...
def test_match_expected_string():
    match = Match(
        src=LPM(IPv4Address("192.168.0.0"), prefix=24),
        dst=LPM(IPv4Address("172.16.0.0"), prefix=16),
...
def test_match_with_subset_lpm_is_not_equal():
    superset = Match(
        src=Exact(PortId(1)), 
        dst=LPM(IPv4Address("192.168.0.0"), prefix=16))
    subset = Match(
        src=Exact(PortId(1)), 
        dst=LPM(IPv4Address("192.168.42.0"), prefix=24))
...
def test_match_difference_multiple_lpm():
    match1 = Match(
        src=LPM(IPv4Address("192.168.0.0"), prefix=16),
        dst=LPM(IPv4Address("192.168.42.0"), prefix=24))
    match2 = Match(
        src=LPM(IPv4Address("192.168.32.0"), prefix=24),
        dst=LPM(IPv4Address("192.168.0.0"), prefix=16))
    expected = Match(
        src=LPM(IPv4Address("192.168.32.0"), prefix=24),
        dst=LPM(IPv4Address("192.168.42.0"), prefix=24))
