...
def test_equal_match_is_superset():
    match = Match(
        src=LongestPrefixMatch(IPv4Address("192.168.1.42"), prefix=32), dst=Ternary(PortId(53))
    )
    assert match >= match
...
def test_match_equal_exact_match():
    match = Match(src=Exact(PortId(1)), dst=Exact(PortId(2)))
    assert match == match
...
def test_match_partially_equal_is_not_equal():
    match_a = Match(src=Exact(PortId(1)), dst=Exact(PortId(2)))
    match_b = Match(src=Exact(PortId(3)), dst=Exact(PortId(2)))
...
def test_match_with_subset_lpm_is_not_equal():
    superset = Match(
        src=Exact(PortId(1)), dst=LongestPrefixMatch(IPv4Address("192.168.0.0"), prefix=16)
    )
    subset = Match(
        src=Exact(PortId(1)), dst=LongestPrefixMatch(IPv4Address("192.168.42.0"), prefix=24)
    )
...
def test_match_difference_lpm_and_exact_superset_first():
    match1 = Match(
        src=LongestPrefixMatch(IPv4Address("192.168.0.0"), prefix=16), dst=Exact(PortId(42))
    )
    match2 = Match(
        src=LongestPrefixMatch(IPv4Address("192.168.32.0"), prefix=24), dst=Exact(PortId(42))
    )
    expected = Match(
        src=LongestPrefixMatch(IPv4Address("192.168.32.0"), prefix=24), dst=Exact(PortId(42))
    )
...
def test_match_difference_raises_if_not_equal():
    match1 = Match(src=Exact(PortId(65)), dst=Exact(PortId(42)))
    match2 = Match(src=Exact(PortId(65)), dst=Exact(PortId(32)))
...
def test_match_with_equal_and_lpm_is_subset():
    match_a = Match(
        src=Exact(PortId(42)), dst=LongestPrefixMatch(IPv4Address("192.168.0.0"), prefix=16)
    )
    match_b = Match(
        src=Exact(PortId(42)), dst=LongestPrefixMatch(IPv4Address("192.168.42.0"), prefix=24)
    )
